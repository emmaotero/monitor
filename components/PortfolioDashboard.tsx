...
export default function PortfolioDashboard({ portfolio, marketData, profile }: any) {
  ...
  const mepData = marketData.find((m: MarketData) => m.key === 'dolar_mep')
  const riesgoData = marketData.find((m: MarketData) => m.key === 'riesgo_pais')
  const mervalData = marketData.find((m: MarketData) => m.key === 'merval')

  const mep    = mepData?.sell_price || 0
  const riesgo = riesgoData?.value || 0
  const merval = mervalData?.value || 0
  ...
  const signalFor = (returnPct: number) => {
    if (returnPct > 0.08)  return { label: 'COMPRAR', cls: 'badge badge-buy' }
    if (returnPct > 0)     return { label: 'MANTENER', cls: 'badge badge-hold' }
    if (returnPct > -0.05) return { label: 'PRECAUCIÓN', cls: 'badge badge-caution' }
    ...
    return (
      <div className="flex items-center justify-center h-96">
        <div className="text-center glass p-12 max-w-sm">
          ...
          <p className="text-sm" style={{ color: '#64748B' }}>Tu asesor aún no configuró tu cartera. Contactalo para comenzar.</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-8 animate-fadeUp">
      ...
      <div className="grid grid-cols-4 gap-4 animate-fadeUp-2">
        {[
          { label: 'Valor Total', value: `$${fmt(totalValue, 0)}`, sub: `Invertido: $${fmt(totalInvested, 0)}`, color: '#E8EDF5', accent: null },
          { label: 'Ganancia / Pérdida', value: `${totalGain >= 0 ? '+' : ''}$${fmt(totalGain, 0)}`, sub: fmtPct(totalReturn), color: totalGain >= 0 ? '#34D399' : '#F87171', accent: totalGain >= 0 ? '#34D399' : '#F87171' },
          { label: 'Dólar MEP', value: `$${fmt(mep, 0)}`, sub: 'Cotización de referencia', color: '#F5C842', accent: '#F5C842' },
          { label: 'Riesgo País', value: fmt(riesgo, 0), sub: 'EMBI+ Argentina', color: riesgo > 1500 ? '#F87171' : riesgo > 800 ? '#F5C842' : '#34D399', accent: null },
        ].map(({ label, value, sub, color, accent }) => (
          <div key={label} className="glass p-5 rounded-2xl"
               style={{ borderColor: accent ? `${accent}20` : undefined }}>
      ...
      {/* Macro strip */}
      <div className="flex gap-4 animate-fadeUp-3">
        {marketData.filter((m: MarketData) => ['dolar_oficial','dolar_blue','dolar_ccl','merval','sp500'].includes(m.key)).map((m: MarketData) => (
          <div key={m.key} className="glass px-4 py-3 rounded-xl flex items-center gap-3 flex-1">
            <p className="text-xs" style={{ color: '#475569' }}>{m.label}</p>
            <p className="text-sm font-semibold text-white ml-auto">
              {m.buy_price ? `$${fmt(m.sell_price || 0, 0)}` : fmt(m.value || 0, 0)}
            </p>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-3 gap-6 animate-fadeUp-4">
        ...
        <div className="col-span-2 glass rounded-2xl overflow-hidden">
          ...
          <table className="w-full data-table">
            ...
            <tbody>
              {assets.map((asset) => {
                ...
                const isPos = asset.return_pct >= 0
                return (
                  <tr key={asset.id}>
                    <td>
                      <div>
                    ...
                    <td className="text-center text-sm text-white">${fmt(asset.current_value, 0)}</td>
                    <td className="text-center">
                      <span className={`text-sm font-semibold ${isPos ? 'text-positive' : 'text-negative'}`}>
          ...
          <div className="px-6 py-4 border-t flex justify-between items-center"
            ...
            <span className="text-xs font-medium uppercase tracking-wider" style={{ color: '#475569' }}>Total</span>
            <div className="flex items-center gap-8">
              <span className="text-sm text-white font-semibold">${fmt(totalValue, 0)}</span>
        ...
        <div className="space-y-4">
          ...
          <div className="glass rounded-2xl p-5 space-y-3">
            ...
            {[
              ...
              { label: 'Mayor ganancia', value: assets.length ? assets.reduce((m,a) => a.return_pct > m.return_pct ? a : m, assets[0])?.ticker || '—' : '—' },
              { label: 'Mayor pérdida', value: assets.length ? assets.reduce((m,a) => a.return_pct < m.return_pct ? a : m, assets[0])?.ticker || '—' : '—' },
            ].map(({ label, value }) => (
